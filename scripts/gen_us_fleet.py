...
TEMPLATE = """\
# {symbol}: {strategy} on a spread-selected US-stock fleet.
{provenance}# Generated by scripts/gen_us_fleet.py — regenerate rather than hand-edit
# fleet-wide changes.
symbol: {symbol}
timeframe: H1
strategy: {strategy}

{strategy_block}{stoch_block}
breadth:
  use: {breadth_use}
  lookback: {lookback}
  min_net: {min_net}
...
# Per-strategy entry blocks and the OOS evidence that justifies each recipe,
# so a generated config carries its own provenance.
STRATEGY_BLOCKS = {
    "macd": """macd:
  fast: 12
  slow: 26
...
PROVENANCE = {
    "macd": """# Out-of-sample on this fleet WITH 5bp/side costs: base test PnL +276, and the
...
""",
    "bollrci": """# Mean reversion (Bollinger + RCI, book combo 6-6). Out-of-sample on this
# fleet with 5bp/side costs, the TRAIN-selected rci_threshold=60 made +502 on
# the held-out half (268 trades, 56% win) vs the macd incumbent's +276 — the
# opposite P&L shape (many small wins) from the trend book. kairi (6-2) was
# also tested and its train-selected knob was ~flat OOS; it did not ship.
# Re-validate with oos_meanrev --configs before changing any of it.
""",
}
...
def main() -> None:
    ...
    p.add_argument("--strategy", default="macd", choices=("macd", "bollrci"),
                   help="recipe to generate: the trend incumbent (macd) or the "
                        "OOS-validated mean-reversion book (bollrci, thr=60). "
                        "Run separate fleets on separate accounts — they share "
                        "risk.max_total_positions counting if co-hosted")
    p.add_argument("--rci-threshold", type=float, default=60.0,
                   help="bollrci only; 60 is the train-selected, OOS-passing "
                        "value — change it only after a fresh oos_meanrev run")
    ...
    if args.symbols_from:
        ...
        src = sources[0]
        src_h4 = {
            "macd": src.macd.use_h4_filter,
            "kairi": src.kairi.use_h4_filter,
            "bollrci": src.bollrci.use_h4_filter,
        }.get(src.strategy, True)
        if not args.no_h4_filter and not src_h4:
...
def _write_fleet(universe, spreads, args) -> None:
    ...
    for i, symbol in enumerate(universe):
        strategy_block = STRATEGY_BLOCKS[args.strategy].format(
            use_h4="false" if args.no_h4_filter else "true",
            rci_threshold=f"{args.rci_threshold:g}",
        )
        text = TEMPLATE.format(
            symbol=symbol,
            strategy=args.strategy,
            strategy_block=strategy_block,
            provenance=PROVENANCE[args.strategy],
        ...
        path = out / f"{args.strategy}_{_slug(symbol)}.yaml"
